...
function ShowAndUpdateDetails() {
  ...
  useEffect(() => {
    const applicant_data = async () => {
      if (consumerId) {
        let result = await handleGetApi("https://htsanyojanuat.mpcz.in:8088/ht_load_change/get-load-change-applications/", consumerId);
       console.log(result)
        setHtConsumer(result.data[29])
        // setFormData(result.data[29])
        console.log(formData,"formDatajj")
      }
    }
    applicant_data();
  }, []);
